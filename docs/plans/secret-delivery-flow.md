...
- [ ] T4. Path-scoped commits (index checked empty first) and pushes in both repos — done when: each commit holds only this work — check: `git show --stat HEAD`
- [ ] T5. Live — done when: the MyVault Worker is deployed with migration 0004 applied and a real delivery against the live Worker was collected — check: `manual: live deposit + collection recorded in the Ledger`
