...
- [x] 13. Demo seed and e2e: one multi-repository demo task appended after the existing seeds; e2e for the New Task flow and Changes sections; full matrix in both themes — done when: the Playwright suite passes — check: `pnpm build && pnpm e2e`
- [x] 14. Real agent run: one real Claude Code task across two scratch repositories — done when: the agent edited both folders, checks ran in each, both task branches carry a commit and the user folders are untouched — check: `manual: task id, branches and commits recorded in the Ledger`
- [x] 15. Docs: workflow-engine, checkpoints, git, tool-system, credential-broker, chairman, dashboard, orchestrator, remote-node system docs and PLAN.md §15/§26 describe the shipped behaviour with current Last verified dates — done when: docs guard passes and each file names the multi-repository behaviour — check: `pnpm docs:guard && git diff --stat docs/ PLAN.md`
...
- [x] T2. Similar-issue sweep — done when: every remaining reader of `task.repositoryId`, `task.git` and `taskWorkdir` was searched and each is either multi-repository aware or deliberately primary-only — check: `manual: list what was searched and what was found`
...
- 2026-09-24 — step 13 — full Playwright suite 99 passed (7.7 min) on ACC_E2E_PORT=4397, chosen so it cannot collide with another session using the default 4391. The e2e caught a real accessibility bug: the "Also work in" picker had no accessible name while wrapped in a div inside Field (Field labels only its direct child); fixed by making the picker the direct child and the chosen list a sibling. Demo seeds orders-api + storefront and one completed task across them, last, so earlier task ids are unchanged
- 2026-09-24 — step 14 — real run on a separate orchestrator built from this branch (port 4398, its own data folder; the live one and its database untouched), Claude Code 2.1.280 for every role, Quick Change, unsupervised. TASK-0001 across lib + app: both agent runs had cwd <data>/workspaces/TASK-0001; lib/index.js renamed greet→sayHello (function and export), app/main.js now calls lib.sayHello; "lib · unit tests" and "app · unit tests" each passed (1 passed (1)); commit 5be3a34 on lib ai/TASK-0001-… and 9a159a6 on app ai/TASK-0001-…; both user trees still on main with their original files (app NOTES.local.md still untracked); worktrees and the task workspace removed; final report READY with a Git section per repository. Codex not exercised (blocked by CLI version/credits per the V1 notes) — unverified
- 2026-09-24 — step 14 — seen in passing: the report redactor replaced the 36-character session folder name inside the scratch path with [REDACTED] (a UUID-shaped segment), which is pre-existing behaviour, not this change
- 2026-09-24 — before step 15 — rebased the branch onto origin/main (c532247) early, so the docs could name the final migration number: another session shipped migration 12 (connected apps), so this plan's migration is **13** (test renamed v12 → v13). Conflicts resolved keeping both sides: report.ts/engine.ts (browserRechecks and repositories both passed to buildFinalReport), context.ts (role prompts v4 gives every role the diff, so the workspace diff is built for every role too), TaskDetailPage (connected-app origin badge kept next to the repository names). After the rebase: typecheck, lint and 68 targeted tests pass
- 2026-09-24 — T5 constraint — local main carries two unpushed audit commits (2a6188f, 54a06da) that must not be pushed (public repository, must-fix list open). This branch is pushed as HEAD:main on top of origin/main, never local main
- 2026-09-24 — step 15 — one new system doc (multi-repository-tasks.md, indexed) plus a short pointer in the section of each doc owning an affected behaviour; docs guard 0 failures. verified_at of the touched docs was not bumped: the guard's staleness warnings there come from other sessions' commits I did not re-read
- 2026-09-24 — T5/T6 coordination — session ai-development-control-center-f7 is fixing the audit findings in the shared tree with unpushed commits and asked that local main NOT be merged while it works; it will rebase onto origin/main itself after this push, and it chooses the moment for a live restart with migration 13. So T6 depends on that session: after the push I notify it and ask for the restart
- 2026-09-24 — T1 — two independent read-only reviewers (engine/tools; API/UI/shared). Fixed: Overview crash on records without repositories (field made optional in the shared type); cloud task.start of a multi-repository task refused (guard gets isMultiRepositoryTask); linked Git updates no longer bump tasks.version (would make Chairman decisions and remote commands stale); Source Control no longer labels your own work in a repository as an unfinished workspace task's; the Source Control test now asserts activeTask, attributionTaskId null for your own file, and commit attribution in a linked repository's history; Changes tab drops a stale selection; New Task sends only repositories it still shows; per-repository nested file lists with labelled groups; banner names the repositories; Execution tab reports the workspace; task + linked rows written in one transaction; a rollback failing half-way inside a repository puts that repository back too (test fails after real changes) and otherwise names the safety checkpoint; Git refused at the workspace root plus GIT_CEILING_DIRECTORIES for tool processes there; environment discovery reads the primary worktree's status; Git cwd may only name the repository root (no nested repositories); a leftover folder's branch is removed on retry; every repository gets its dependency install on resume; resume parks when a worktree was removed; files at the workspace root are reported and the task is never READY; the gate treats an unreadable repository as unknown; crash-retry message no longer claims nothing changed; doc wording (credentials at the root = not limited to any repository; cloud offline filter; additive null fields)
- 2026-09-24 — T1 — left, with reasons: the plan text names a refusal code MULTI_REPOSITORY_LOCAL_ONLY, the shipped guard returns the dispatcher's standard REMOTE_FORBIDDEN with its own message (Context is verbatim; this line corrects it); step 11's check listed api/source-control/remote test files but the assertions live in multi-repo.test.ts (those suites still ran green unmodified); simulated-agent folder inference affects demo/tests only; App-check event order and the extra repositoryId in single-repository GIT_COMMIT/WORKTREE_REMOVED data are additive; "Open file" opens your own checkout, as for isolated single-repository tasks; the full repository list sits in title on list rows but in full on the task header
- 2026-09-24 — T1 — found for later: (a) Source Control labels your own work as an unfinished ISOLATED single-repository task's (pre-existing; fixed here only for workspace tasks so single-repository behaviour is unchanged); (b) a dependency install during workspace preparation ignores cancel (up to 20 min per repository); (c) agent CLIs' own git at the workspace root is not ceiling-limited (no env hook in AgentExecutionInput) — safe with the default data folder, which is in no repository; (d) whether a rollback can reach a finished isolated task, whose workdir then falls back to your checkout (pre-existing for single-repository tasks)
- 2026-09-24 — T2 — searched apps/orchestrator/src (after the rebase, so including connected apps, learning and role prompts v4) for taskWorkdir(, task.repositoryId and task.git.* readers. Every taskWorkdir caller is either a single-repository fallback behind a multi-repository branch (checkpoints, evidence, context, complete, gateCommand, runners units, tooling status and prepareWorktree, routes) or the accessor itself. Deliberately primary-only: usage projectId, stage gate auto-approve (the task already holds the strictest level), cancel cleanup scope, the Chairman contract (names joined), the environment.discover tool (delegates to discoverEnvironment, which handles workspaces), protectedPaths (empty for isolated). Found for later: the learning loop (learning/service.ts) reviews, stores lessons for and loads learned skills of the primary repository only, so lessons of a linked repository do not reach a task across repositories
- 2026-09-24 — T3 — on the rebased branch after the review fixes: pnpm check exit 0 (typecheck, lint, docs guard 0 failures / 25 staleness warnings, vitest 63 files 756 passed); pnpm build then pnpm e2e (ACC_E2E_PORT=4397) 104 passed in 8.9 min, both themes
