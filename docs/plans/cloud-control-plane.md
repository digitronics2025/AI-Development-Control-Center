...
- [ ] T6. Confirmed live — done when: the deployed Worker is observed on its hostnames (health, fail-closed control host, node connected) — check: `pnpm cloud:smoke`
- [ ] T7. A claim registered for this change — done when: the repo's claims register holds an entry, or this step says "no observable outcome" with the reason — check: `manual: name the claim and its deadline, or why none`
