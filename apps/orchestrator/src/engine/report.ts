...
export function buildFinalReport(input: ReportInput): ReportResult {
  ...
  const lines = [
    ...
    '',
    `**${task.id} · ${task.title}**`,
    `Repository: ${repo.name} (${repo.path}) · Workflow: ${task.workflow.name} · Mode: ${task.mode === 'discuss' ? 'Discuss First' : 'Autopilot'}`,
    '',
    '## Requested',
    '',
    ...
    '## Review',
    '',
    lastReview ? (lastReview.verdict === 'PASS' ? 'Passed' : 'Issues remain') : 'No review stage',
