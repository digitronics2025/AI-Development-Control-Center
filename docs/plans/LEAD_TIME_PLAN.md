...
- [x] 11. Credentials and configuration (§4 step 5) → deferred: blocked on the operator — the token must be made and deployed by a Control Center task with the operator's typed Level 5 approval, and the recipient is the operator's own messenger address (see Ledger) — token generated and saved to MyVault, token and recipient deployed to the messenger Worker through secret-custody, Phone alerts filled in with the cloud dashboard as open URL, orchestrator restarted on the new build with -Drain — done when: Send a test reports sent — check: `manual: the Send a test result in Settings`
