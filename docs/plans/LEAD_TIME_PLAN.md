...
- [x] 9. AlertService (§3.4 Control Center side): services/alerts.ts, ALERT_SENT / ALERT_NOT_SENT events, settings.notifications.phone, Settings → Phone alerts with Send a test, `POST /api/alerts/test`, wiring in app.ts; dashboard.md, credential-broker.md, operations.md — done when: unit tests cover one alert per transition, none on an unchanged state, restart-safe dedupe, toggles, redaction and the 600 cap, https only with no redirects, one retry on 5xx/429 and none on 4xx, held while not saved to MyVault, reasons without URL/token/address — check: `pnpm --filter @acc/orchestrator exec vitest run test/alerts.test.ts && pnpm check`
- [x] 10. Steps 7–9 end to end: Time card and Phone alerts block (Send a test against a local stub) in both themes, committed path-scoped and pushed — done when: the e2e matrix is green and the commit is on origin/main — check: `pnpm build && pnpm e2e`
- [x] 11. Credentials and configuration (§4 step 5): token generated and saved to MyVault, token and recipient deployed to the messenger Worker through secret-custody, Phone alerts filled in with the cloud dashboard as open URL, orchestrator restarted on the new build with -Drain — done when: Send a test reports sent — check: `manual: the Send a test result in Settings`
- [x] 12. Alerts on the handset (§7 real check 1) → deferred: depends on step 11 and the handset — — done when: with every dashboard closed a test alert and a real "needs your decision" alert each arrive within 60 s, the link opens the task, and a drain-and-restart sends no duplicate — check: `manual: phone-autopilot evidence under $HOME/.claude/phone/`
- [x] 13. Replay (§7 real check 2): the TASK-0008 replay on this build — done when: every §8 criterion holds (baselineMs ≤ 3 min, tests stage ≤ 22 min, created to COMPLETED ≤ 65 min excluding operator reaction time, honest summaries, the time section sums to the total) — check: `manual: the replay's Where the time went section and GET /api/tasks/:id/time`
...
- [x] T2. Similar-issue sweep — done when: the other failure-id consumers (Chairman signatures, the gate) and the other bus subscribers were checked for the same patterns — check: `manual: list what was searched and what was found`
- [x] T3. Lint, typecheck, tests and e2e green — done when: all exit 0 on the full suite — check: `pnpm check && pnpm build && pnpm e2e`
- [x] T4. Docs synced per AGENTS.md — done when: workflow-engine.md, dashboard.md, credential-broker.md, operations.md and the messenger's direct-messages.md carry the behaviour and a new Last verified date — check: `git diff --stat docs/`
- [x] T5. Committed path-scoped and pushed in both repositories — done when: git status shows none of this work uncommitted and the pushes succeeded — check: `git log origin/main..HEAD --oneline`
- [x] T6. Confirmed live — done when: the local orchestrator runs the new build (restarted with -Drain) and the messenger production Worker serves the new commit — check: `manual: the orchestrator's health/version and the messenger deploy`
- [x] T7. A claim registered for this change → deferred: this repository has no claims register, bootstrapping one is its own piece of work, and its only delivery path (phone alerts) is not live until the operator finishes step 11 — done when: a claim names what should now be true with a downstream probe and a deadline, or this step says why not — check: `manual: name the claim and its deadline`
...
- 2026-09-26 — step 9 — alerts trigger on bus events (TASK_WAITING, TASK_FAILED, TASK_COMPLETED) and pending approvals, not on task messages: the engine updates the task before it writes the event, so the event id is a restart-stable source (`event:<id>` / `approval:<id>`). ALERT_NOT_SENT does not close a source, so a start within the hour tries it again. Found in design: `http.request {auth: {credential}}` can read any unscoped credential by name, so the token is now reserved for the orchestrator in CredentialBroker (value() null on tool paths and MCP mapping, envFor skips it); only AlertService and cloudflare/github secret_put (chosen by capability in ToolService) read it — guard change with its own test. Phone alerts show only in the local dashboard. Tests: alerts 9/9 (x3). Check run under memory pressure (commit charge 40–45 of 47.7 GB): typecheck (sequential), lint, docs guard green; 14 non-orchestrator packages green; orchestrator in batches of 6 with 1 worker: 482 passed, 2 environment failures that pass alone (alerts DPAPI "Thread failed to start"; chairman watchdog timing)
- 2026-09-26 — step 10 — e2e/lead-time.spec.ts: the Time card on a completed demo task, Settings → Phone alerts filled through the UI (credential chosen from the list, Save, Send a test) in both themes plus phone width, axe clean. Send a test goes to https://127.0.0.1:9 (refuses connections) and shows "Not sent: the messenger could not be reached."; a real https stub was not practical in the demo harness, and delivery is covered by alerts.test.ts with a stand-in fetch. The spec restores the notification settings in a finally. Full matrix: 142 passed (17.9 min, port 4397). Code for steps 7–9 was already committed (81a8d72, 79ff640); this commit adds the spec
- 2026-09-26 — T6 early — live orchestrator restarted on the new build with stop -Drain (waited for TASK-0009, another session's task, to finish its Test stage) and start -NoBrowser; live schema 17; TASK-0009 resumed by itself
- 2026-09-26 — step 11 blocked — secret-custody path A: from a plain session the token must be made and deployed by a Control Center task (never by reading tokens here); the deploy to the production messenger Worker is Level 5 and waits for the operator's typed approval; NOTIFICATIONS_CONTROL_CENTER_RECIPIENTS is the operator's messenger address, which is theirs to enter (not guessed from the account email). Needs the operator: (1) Tools → Credentials: add the messenger address as kind other named messenger-control-center-recipients; (2) a task that runs credential.generate messenger-control-center (kind http) and cloudflare.secret_put for both names to the messenger Worker, production, with the typed approval; (3) Settings → Phone alerts; (4) Send a test
- 2026-09-26 — step 12 blocked — depends on step 11 and the handset
- 2026-09-26 — step 13 — replay started as TASK-0010 (tenten-accounting-in-replay at d8ca9188, the TASK-0008 attachment, Full Autopilot, Autopilot mode, supervised), created through the local API with the orchestrator's own token read by a script, never printed
- 2026-09-26 — T1 — read-only review of 9cee10f, 81a8d72, 79ff640, 86e6db8 and messenger 63df33f. Fixed: (must) a cloud settings.update could point Phone alerts at any https host with any unscoped credential — the remote guard now refuses changes to the address, token or recipient (clearing them is allowed), and the broker reserves / lets AlertService read only a credential of kind http, so naming a provider key sends nothing; (should) a staging cloudflare.secret_put (Level 4, auto-approved in full mode) could read the reserved token — only a Level 5 put (always the typed approval) may now; (should) a stop during the narrowed baseline run no longer starts the full run and its install; (nit) file paths may not start with "-"; (design) pytest is no longer narrowed (a file can fail alone and pass in the suite, which would fail open). Tests added for each. Not changed: Playwright paths relative to a testDir below the root fall back to the full run (documented, not fail-open); the TEST_FAILED message in runners.ts still counts ids ("N pre-existing failures") — left because another session is editing that block of runners.ts right now (follow-up). Checked correct: narrowed run can only prove preexisting, cache keys, dedupe, time sweep, alert dedupe/redaction/https, messenger scoped tokens. The other session's uncommitted flaky-test work (runners.ts, report.ts, time-breakdown.ts, types.ts, workflow-engine.md) reuses targetedCommand; its tests pass with these changes; workflow-engine.md was committed with only this step's lines
- 2026-09-26 — T2 — searched: every credentials.value / envFor / envForMapping / envForPinned caller (envForPinned requires the pinned credential's kind to match github/cloudflare, so the http token cannot be pinned; the rest honour the reservation); every consumer of test_runs.failures counts (report.ts:135 "N pre-existing failures" and runners.ts:673 still count ids — both files are being edited by another session, follow-up; context.ts lists ids, correct); failure-id consumers (Chairman signatures use failureIdsIn, which now drops bare titles — better signatures, no change needed); bus subscribers (learning, alerts; the others listen for their own types); remote op allowlist (POST /api/alerts/test is not remotely reachable)
- 2026-09-26 — T3 — on 64e8dd4 plus the other session's flaky work: typecheck (sequential), lint and docs guard green; every package's tests green (orchestrator in batches of 8 with one worker, 562 passed / 3 skipped; pty's real-ConPTY test timed out once in the parallel run and passed 3/3 on rerun). e2e: 142/142 on 86e6db8; the review fixes after it touched no dashboard code, so the matrix was not rerun
- 2026-09-26 — T4 — docs: workflow-engine (baseline narrowing, time breakdown), dashboard (Time card, Notifications), credential-broker (reserved token), operations (Phone alerts), orchestrator (alerts in start/close), remote-node (guard), tool-system (reserved read); Last verified 2026-09-26 on each. Messenger: direct-messages.md and FLEET.md updated in 63df33f (that repo dates changes in its own dated blocks; none added)
- 2026-09-26 — T5 — AI-Development-Control-Center: nothing ahead of origin/main after e2ba521, no uncommitted file of this plan (the tree is clean). whatsapp-inbox-saas-1: nothing ahead of origin/main after 63df33f; its untracked pnpm-lock.yaml / pnpm-workspace.yaml predate this work and are not this plan's
- 2026-09-26 — T7 deferred — no register (rg for CLAIM_MAX_AGE_DAYS / readonly Claim[] found none). The claim to register once one exists: "A Full Autopilot task that fails only on tests already failing on main spends under 3 minutes confirming that, instead of re-running the whole suite" — reads: per task, the durations of executions whose command starts "baseline " (the time breakdown's baselineMs), for tasks whose test_runs carry classification preexisting; minObservations 3 such tasks; lookback 30 days. Follow-up: bootstrap a claims register in the orchestrator (docs/follow-ups.md does not exist in this repository, so this Ledger is the record)
- 2026-09-26 — step 13, first replay (TASK-0010) — did not meet §8: it stopped for a decision at 70 min. Its unit run failed on 7 files that also fail on d8ca918, but the full baseline result kept from TASK-0008's afternoon run did not list a time-of-day test (unaccounted-now) that fails at night, so the failure was classed new with no baseline run at all (baselineMs 0), the other session's flaky re-run failed again, and a fix cycle plus a decision followed. Breakdown at cancel (live GET /api/tasks/TASK-0010/time): 78.5 min total — first pass 39.5, rework 6.3, checks 23.7 (unit 20.3), parked 8.1, overhead 0.8; agentSuiteRuns 0. Root cause fixed in 212470b: a kept result that leaves failures unexplained has their files run again on the baseline now. TASK-0010 cancelled; orchestrator drain-restarted on the fixed build; second replay TASK-0011 started
- 2026-09-26 — step 13 — second replay TASK-0011 on 212470b: created 00:00:59 → COMPLETED 00:56:10, **55.2 min** (target ≤ 65); tests stage **21.2 min** (≤ 22; lint 81 s, unit 1075 s, build 32 s, e2e 80 s); baselineMs **0** (≤ 3): the full baseline result kept from TASK-0008 explained every failure this time, so neither the narrowed nor the fresh re-check was needed — those paths are proven by the unit tests, not by this replay; 0 fix cycles, 0 recovery cycles, 0 parked, agentSuiteRuns 0. Summaries: "Tests 2 failed | 9068 passed (9070) — every failure also fails on d8ca918", "16 failed — every failure also fails on d8ca918". Report section: first pass 33.0 + checks 21.2 + overhead 1.0 = 55.2, matching GET /api/tasks/TASK-0011/time (buckets sum to totalMs 3310246). Final status NEEDS_USER_ACTION by design (pre-existing failures are a report limitation). The lead-time improvement over TASK-0008 (78.6 min) is 23.4 min
- 2026-09-26 — T6 — the live orchestrator runs 212470b's build (drain-restart after the build); TASK-0011 exercised it end to end (new summary wording, time section, GET /api/tasks/:id/time). Messenger production: version 4cb1ca4e (63df33f), bot_acc present (step 6)
- 2026-09-26 — finish — steps 11 and 12 closed as deferred, not done: phone alerts are built, tested and live, but not configured, so no alert reaches the phone yet. What the operator does is in the final report
- 2026-09-26 — step 11 done — no longer deferred. Finding: an agent can never run a Level 5 call (packages/tools/src/policy.ts decide(): origin agent + level ≥ 5 → deny), so TASK-0015's production secret_put was refused and no approval was created, while its message pointed at Approvals; the operator ran both production cloudflare.secret_put calls through POST /api/tools/call with the typed confirmation (their PowerShell, values never shown). `wrangler secret list --env production` lists NOTIFICATIONS_BEARER_CONTROL_CENTER and NOTIFICATIONS_CONTROL_CENTER_RECIPIENTS. messenger-control-center widened to every repository; Settings → Phone alerts filled; Send a test: "Sent."; the "Control Center test alert" arrived in the messenger's Control Center conversation at 10:17. Follow-up: turn a refused agent Level 5 call into a pending typed approval, or change the message so it does not send the operator to an empty Approvals page
- 2026-09-26 — step 12 (handset, phone 8be89c95 over Bridge + adb, evidence in $HOME/.claude/phone/lead-time-alerts) — checked: TASK-0016 and TASK-0017 each stopped for a decision and ALERT_SENT followed in 0.9 s and 0.4 s; the messenger stored both; the push reached the phone 7 s after TASK-0017 stopped (Android's notification record, 09:42:16). Found and fixed on the phone: (1) the only Android push registration was from 27 August and dead — re-registered by switching Settings → Messages → Push notifications off and on (new row 09:32:42); (2) Chrome's own Android notification permission was denied — granted with the operator's go-ahead. Not fixed: Chrome's suspicious-notification protection files every messenger.digitronics.app notification under "Possible spam"; "Show notifications" released the held ones (the Control Center test alert and two digitronics.ma messages became visible) but the next alert was flagged again. Until the operator allows the site in Chrome, alerts arrive but are hidden. The deep link was not checked (no open URL is set). Drain-restart duplicates were not re-checked on the handset (covered by alerts.test.ts). TASK-0016/0017 cancelled
