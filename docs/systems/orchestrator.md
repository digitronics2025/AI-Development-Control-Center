...
`credential(.deleted)`, `recovery`, `escalation`, `connectedApp` (never relayed to the cloud), and Ask's
`ask.thread(.deleted)`, `ask.message`, `ask.delta` (never relayed). `terminal.output` goes only
